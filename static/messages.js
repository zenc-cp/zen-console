        ...
        setTimeout(async()=>{
          try{
            ...
            if(st.active){
              ...
              _wireSSE(new EventSource(new URL(`${(location.pathname.match(/^\/[^\/]+/)||[''])[0]}/api/chat/stream?stream_id=${encodeURIComponent(streamId)}`,location.origin).href,{withCredentials:true}));
              return;
            }
          }catch(_){}
          _handleStreamError();
